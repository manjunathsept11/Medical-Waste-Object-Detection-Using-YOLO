...
def playAlarm(result):
    engine = pyttsx3.init();
    for i in range(len(result)):
        #engine.say(result[i]['label'] + " Found with " + str(int(result[i]['confidence']*100)) + "Percent confidence");
        engine.say(result[i]['label']);
    engine.runAndWait();
    return
...
def calc():
    ...
    options = {'model': 'cfg/yolo.cfg','load': 'bin/yolov2.weights','threshold': 0.2,'gpu':1.0}
    tfnet = TFNet(options)
